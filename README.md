...
│   │   ├── ZN1001v2-3dnew.fbx   # 保留的模型源文件，不进入当前 Web 构建
│   │   └── ZN1001v2.fbx         # 当前 Web 页面加载的模型
...
| 模型 | 实体构件 | 说明 |
| --- | ---: | --- |
| `ZN1001v2.fbx` | 145 | 当前唯一加载的原始导出模型；加载时自动过滤 61 条辅助中心线 |
...
`web/src/twin.js` 使用 Vite 的 `?url` 方式导入 `models/fbx/ZN1001v2.fbx`。页面中的模型栏目固定显示为 `ZN1001`，不再提供模型切换选项。

RVT 文件仅作为 BIM 源模型保存，浏览器无法直接加载。当前 Web 端统一使用 FBX；如需更新模型，应从源模型重新导出 FBX，并更新 `web/src/twin.js` 中的模型导入。
