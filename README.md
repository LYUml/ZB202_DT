...
- 支持简体中文、繁體中文和 English；设备详情与房间视图继承设备总览页选择的语言。
...
```mermaid
%%{init: {"theme": "base", "flowchart": {"curve": "basis", "nodeSpacing": 36, "rankSpacing": 48}, "themeVariables": {"fontFamily": "Inter, PingFang SC, Microsoft YaHei, sans-serif", "fontSize": "14px", "lineColor": "#94a3b8"}}}%%
flowchart LR
...
语言由 `?lang=zh`、`?lang=zh-Hant` 或 `?lang=en` 控制，并保存到浏览器。设备总览提供简体中文、繁體中文和 English 三种选择，进入设备详情或房间视图时会自动传递当前语言。

日间／夜间模式在设备总览页的语言选择器右侧切换，并通过 `zb202-theme` 保存为全局偏好；设备详情和房间视图会自动继承。
